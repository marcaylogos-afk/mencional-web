...
export const StartSessionButton: React.FC<StartSessionButtonProps> = ({ onStart }) => {
  ...
  const handleAction = () => {
    if (isAdmin) {
      ...
      if (onStart) {
        ...
        // Enlace directo al checkout sincronizado: $50 MXN (Protocolo v2.6)
  ...
  return (
    <div className="w-full flex justify-center py-8 px-4 relative z-20 italic">
      <motion.button
      ...
      >
        ...
        <div className="flex flex-col items-start z-20 text-black text-left">
          <div className="flex items-center gap-4">
            ...
            <span className="font-[1000] text-2xl md:text-4xl tracking-tighter leading-none uppercase">
          ...
          <div className="flex items-center gap-3 md:gap-4 mt-2.5">
            <span className={`text-[10px] font-black uppercase tracking-[0.4em] md:tracking-[0.6em] ${isAdmin ? 'opacity-90' : 'opacity-60'}`}>
              {isAdmin ? "UNLIMITED_BYPASS_ACTIVE" : "SESIÓN_30MIN: $50.00 MXN"}
            </span>
