...
interface TimerDisplayProps {
  ...
  accentColor?: string;
  initialMinutes?: number; // Ahora por defecto 60 según nueva estrategia
  onFinish?: () => void;
}
...
export const TimerDisplay: React.FC<TimerDisplayProps> = ({ 
  mode, 
  isTranslating, 
  accentColor = "#00FBFF", // Cyan OLED
  initialMinutes = 60,      // Sesión completa de 1 hora
  onFinish
}) => {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      // Si la sesión global terminó, congelamos el ciclo
  ...
  // 🕒 Manejador de la Sesión Global (1 Hora)
  useEffect(() => {
    if (sessionSeconds <= 0) {
      onFinish?.();
      return;
  ...
  const isCriticalTime = sessionSeconds <= 300; // 5 min: Pulso Rojo (Pago)
  ...
  return (
    <div 
      className="flex flex-row items-center gap-8 select-none bg-black/95 p-5 pl-8 rounded-[3rem] border-2 backdrop-blur-3xl shadow-[0_40px_100px_rgba(0,0,0,1)] transition-all duration-700 italic"
      ...
      <div className="flex flex-col gap-1 pr-4">
        <div className="flex items-center gap-2 mb-1">
          {isWarningTime ? (
            ...
            >
              ...
              <span className={`text-[8px] font-black uppercase tracking-widest italic ${isCriticalTime ? 'text-rose-500' : 'text-orange-500'}`}>
                {isCriticalTime ? 'Block_Expiring_Soon' : 'Session_End_Near'}
              </span>
            </motion.div>
          ) : (
          ...
          {isTranslating && (
            <div className="flex items-center gap-2 px-3 py-1 bg-[#39FF14]/10 border border-[#39FF14]/30 rounded-full">
              ...
              <span className="text-[8px] font-black text-[#39FF14] uppercase tracking-widest italic">Neural_Burst</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-baseline gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[8px] font-bold text-zinc-800 uppercase tracking-widest mt-1 italic">V26_60M</span>
          </div>
        </div>
        ...
        <div className="flex gap-1 mt-4 items-end h-4">
          {[...Array(12)].map((_, i) => (
            <motion.div 
              ...
              style={{ boxShadow: (progress / 8.33) > i ? `0 0 10px ${accentColor}44` : 'none' }}
            />
          ))}
        </div>
      </div>
