 ...
 * Ubicación: /src/components/ia/PreFlightPanel.tsx
...
export const PreFlightPanel: React.FC<PreFlightPanelProps> = ({ 
  ...
  const handleComplete = () => {
    // Sincronización con el núcleo global de la sesión
    onConfirm({
      name: name || "User_Node",
      privacy,
      audioLevel,
  ...
  return (
    <motion.div 
      ...
      className="w-full max-w-2xl mx-auto select-none p-4"
    >
      <div className="relative overflow-hidden bg-black border border-white/5 rounded-[3rem] md:rounded-[3.5rem] p-8 md:p-14 shadow-[0_50px_100px_rgba(0,0,0,1)]">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[7px] text-zinc-600 font-black uppercase tracking-[0.3em] mt-2 italic flex items-center gap-2">
                <Cpu size={8} /> Stable_Build_v16.0
              </p>
            </div>
          </div>
          <div className="hidden md:flex items-center gap-2.5 px-4 py-2 rounded-full border border-white/5 bg-zinc-950/50">
            <div className="w-1.5 h-1.5 rounded-full bg-[#00FBFF] animate-pulse shadow-[0_0_8px_#00FBFF]" />
          ...
          <section className="space-y-4">
            ...
            <div className="relative group">
              <input 
                ...
                className="w-full bg-zinc-950 border-2 border-white/5 rounded-[2rem] px-8 py-6 text-base md:text-lg font-black text-white placeholder:text-zinc-900 focus:outline-none focus:border-[#00FBFF]/30 transition-all duration-500 uppercase italic tracking-widest"
              />
              ...
              return (
                <button 
                  ...
                  className={`p-6 rounded-[2.5rem] border-2 transition-all duration-500 flex flex-col gap-4 items-start relative overflow-hidden group ${
                    isSelected 
                    ? isGhost ? 'bg-[#00FBFF] border-transparent text-black shadow-[0_0_30px_rgba(0,251,255,0.2)]' : 'bg-white border-transparent text-black shadow-[0_0_30px_rgba(255,255,255,0.1)]'
                    : 'bg-zinc-950 border-white/5 text-zinc-600 hover:border-white/10'
                  }`}
                >
                  <mode.icon size={16} className={isSelected ? 'animate-bounce' : 'opacity-40'} />
        ...
        <div className="mt-14">
          <button 
            ...
            disabled={!name}
            className={`w-full py-8 rounded-[2.5rem] flex items-center justify-center gap-4 font-[1000] uppercase tracking-[0.6em] text-[11px] transition-all duration-700 shadow-2xl relative overflow-hidden group ${
              name 
              ? 'bg-white text-black hover:bg-[#00FBFF] active:scale-[0.98]' 
              : 'bg-zinc-950 text-zinc-800 cursor-not-allowed border border-white/5'
            }`}
          >
            {name && (
