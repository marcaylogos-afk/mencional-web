...
const TrendingNube: React.FC = () => {
  ...
  const handleTrendSelect = (trend: TrendItem) => {
    ...
    // Bypass Maestro: Redirección directa para administradores
    if (isAdmin) {
  ...
  return (
    <section className="w-full py-24 px-6 md:px-12 bg-[#050505] rounded-[3.5rem] border-2 border-white/5 shadow-2xl overflow-hidden relative">
      ...
      >
        <div className="flex items-center gap-10">
          ...
          <div className="flex items-center gap-3 bg-zinc-950 px-6 py-2.5 rounded-full border border-orange-500/30">
            <Zap size={14} className="text-orange-500 fill-orange-500 animate-pulse" />
            <span className="text-[10px] font-black tracking-[0.5em] uppercase text-orange-500 italic">
              NEURAL_TRENDS_LIVE
            </span>
          </div>
        ...
        <div className="text-center space-y-4">
          <h2 className="text-white text-4xl md:text-6xl font-[1000] tracking-tighter uppercase italic leading-none">
            Nube de Sincronía
          </h2>
          ...
          {activeTrends.map((trend, index) => (
            ...
            >
              <div className={`
                ...
                ${trend.intensity > 1.4 
                  ? 'border-orange-500/50 shadow-[0_30px_70px_rgba(249,115,22,0.15)]' 
                  : 'border-white/5'}
                ...
                <div className="flex flex-col gap-5 items-start">
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[9px] font-black text-orange-500/70 tracking-[0.3em] uppercase italic">
                        #{trend.topic}
                      </span>
                    </div>
                  </div>

                  <p className="text-white font-[1000] text-xl md:text-3xl uppercase italic leading-tight tracking-tighter group-hover:text-orange-400 transition-colors">
                    "{trend.phrase}"
                  </p>
                  ...
                  <div className="flex items-center justify-between w-full mt-2 border-t border-white/5 pt-6">
                    <div className="flex items-center gap-3 text-zinc-600 group-hover:text-zinc-400">
                      ...
                      <span className="text-[8px] font-black tracking-[0.3em] uppercase italic">
                        {trend.usersActive} Nodos
                      </span>
                    </div>
      ...
      >
        <div className="w-full max-w-4xl bg-zinc-950/60 p-12 rounded-[3rem] border-2 border-white/10 backdrop-blur-3xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {[
              ...
              <div key={i} className="p-8 bg-black/40 rounded-[2rem] border-2 border-white/5 hover:border-orange-500/40 group transition-all cursor-pointer">
                <div className="flex justify-between items-start mb-5">
                  <span className="text-[9px] text-zinc-600 font-black uppercase italic group-hover:text-orange-500 tracking-widest">
                    In_{session.time}
                  </span>
                  <div className="w-2 h-2 rounded-full bg-zinc-800 group-hover:bg-orange-500 transition-all shadow-[0_0_8px_rgba(249,115,22,0)] group-hover:shadow-[0_0_8px_rgba(249,115,22,0.6)]" />
                </div>
