...
interface TurnIndicatorProps {
  ...
  isActive: boolean;
  isUserTurn: boolean;
  colorCode: string;    // Rosa, Verde, Azul, Turquesa, etc.
...
export const TurnIndicator: React.FC<TurnIndicatorProps> = ({
  ...
  // 🕒 Alerta crítica configurada a los 5 minutos (300 segundos)
  ...
  // Estilos optimizados para Negro Absoluto OLED
  ...
  return (
    <motion.div
      ...
      animate={{ 
        ...
        backgroundColor: isUserTurn ? "rgba(0,0,0,0.8)" : "rgba(5,5,5,0.4)",
      }}
      transition={{ type: "spring", stiffness: 200, damping: 25 }}
      className={`relative p-7 md:p-10 rounded-[2.5rem] border-2 transition-all duration-700 overflow-hidden backdrop-blur-xl ${
        isUserTurn 
          ? "z-20 shadow-2xl" 
          : "z-10 opacity-40 grayscale-[0.5] blur-[0.5px]"
      }`}
    >
      {/* 🌌 DINÁMICA DE RESPLANDOR OLED */}
      <AnimatePresence>
        {isUserTurn && (
          <motion.div 
            initial={{ opacity: 0 }}
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center gap-2">
              <span 
                className="text-[10px] font-black tracking-[0.3em] uppercase italic"
      ...
      {/* 〰️ PROGRESS BAR: PROTOCOLO 6 SEGUNDOS (Cíclico) */}
        ...
        <AnimatePresence mode="wait">
          {isUserTurn && (
            <motion.div
              ...
              animate={{ width: "100%" }}
              transition={{ 
                duration: 6, // Límite exacto de 6 segundos
                ease: "linear",
                repeat: Infinity 
              }}
              ...
              style={{ backgroundColor: colorCode }}
            >
              {/* Micro-destello en el frente de la barra */}
              <div className="absolute right-0 top-0 h-full w-6 bg-white/50 blur-md" />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
